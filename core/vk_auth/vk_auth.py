...
async def vk_callback(code: str):
    data = requests.get(f'https://oauth.vk.com/access_token?client_id={Config.vk_app_id}&client_secret={Config.vk_secret_key}&redirect_uri={Config.domain+"api/oauth/vk/callback"}&code={code}').json()
    access_token = data['access_token']
    vk_id = data['user_id']
    email = data['email']

    # CALL VK API
    data = requests.post('https://api.vk.com/method/account.getProfileInfo', data={
